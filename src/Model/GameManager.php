...
class GameManager extends AbstractManager
{
    ...
    public function selectAllMyGames(int $id): array
    {
        $query = 'SELECT distinct(g.id) as game_id, g.name, g.type, g.minimum_players_age, g.image, g.id_owner, 
        g.min_number_players, g.max_number_players, g.availability FROM game AS g
        WHERE g.id_owner = :id;';
        $statement = $this->pdo->prepare($query);
        $statement->bindValue(':id', $id, \PDO::PARAM_INT);
        $statement->execute();
    ...
    public function selectMyGamesOnce(int $id): array
    {
        $query = 'SELECT b.id AS borrow_id, b.id_game, b.id_user, b.id_status, u.id AS user_id, 
        u.firstname AS user_firstname, u.lastname AS user_lastname FROM game as g
